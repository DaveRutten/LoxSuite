...
const ICONS = {
  ...
  // Topbar "Support this project" trigger (see partials/head.ejs) — filled, not outline like the
  // rest of this set, since a hollow heart reads as "not favorited yet" elsewhere in the app (the
  // dashboard star uses the same fill-vs-outline convention) and that's not the meaning here. This
  // is the standard Material "favorite" glyph (well-tested, symmetric in a 24x24 grid) rather than
  // a hand-drawn path, since a hand-tuned heart is easy to get visibly lopsided.
  heart: '<path fill="currentColor" stroke="none" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35Z"/>',
  // The AI Assistant's own floating launcher (see partials/ai-chat-widget.ejs) — a speech bubble,
  // distinct from `help` (a plain "?" in a circle, already means "explain this page" elsewhere).
  ...
  // Panel-grid's own "Lock panels" toggle (see partials/panel-grid.ejs) — one icon, outline vs
  // filled (see style.css's own #dash-lock-panels-btn.is-active), same "single icon + fill/color
  // carries the on/off state" convention the favorite star above already uses, rather than a
  // second "unlock" icon swapped in/out.
