// The original gateway/src/db.js schema + all ~45 hand-written SQLite migration functions,
// relocated here VERBATIM (only DB_PATH -> the dbPath parameter, so this no longer depends on a
// module-level constant) as part of splitting db.js into gateway/src/db/ (see that package's own
// index.js for why). This module is SQLite-only and stays that way forever — it only ever needs to
// walk a pre-existing SQLite gateway.db forward through its own history; a fresh install on ANY
// backend (including a fresh SQLite one) uses gateway/src/db/migrations/001_baseline.js instead
// (Phase 2 of the multi-backend rollout — see the project's own db-backend plan). Do not add new
// migrations here once that baseline exists; this file is frozen, kept only so upgrading installs
// have somewhere to replay their history from.
//
// Runs synchronously against a plain, already-open better-sqlite3 Database instance — exactly the
// API this code always used, since it predates the async facade entirely. gateway/src/db/index.js
// opens a temporary raw connection, calls this function once, closes it, then hands off to Knex to
// open the app's real (pooled, async-wrapped) connection to the now-fully-migrated file.
...
function runLegacySqliteSchema(db, dbPath) {
  db.exec(`
  ...
  function checkEmergencyPasswordReset() {
    const resetFilePath = path.join(path.dirname(dbPath), 'reset-password.txt');
  ...
  // The session store switched from better-sqlite3-session-store to connect-session-knex (see
  // db/index.js's own header and server.js) as part of moving the whole app onto an async DB
  // facade — the old store isn't SQLite-only-compatible the way the app itself now needs to be
  // (its `client` option is the raw better-sqlite3 handle the facade no longer exports). The two
  // stores use a differently-shaped `sessions` table (old: sid/sess/expire; connect-session-knex's
  // own: sid/sess/expired, note the 'd' — see its own lib/index.js), and connect-session-knex only
  // creates the table itself if none exists yet, so an old-shaped table left in place would just
  // get used as-is and fail on first read/write instead of being recreated correctly. Detected by
  // the presence of an `expired` column (the new store's own name) — a table that already has one
  // is already correctly shaped (a no-op here on every later boot); a `sessions` table that exists
  // WITHOUT one is the old store's own shape and gets dropped so connect-session-knex creates a
  // fresh one on its next start. Every existing session is invalidated by this exactly once, on the
  // release that introduces it — documented in the CHANGELOG, same one-time cost the emergency
  // password reset flow already deliberately accepts (see checkEmergencyPasswordReset above).
  function migrateSessionsTableForKnexStore() {
    const tableSql = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'sessions'").get();
    if (!tableSql) return; // no sessions table yet at all — connect-session-knex creates one fresh
    const columns = db.prepare('PRAGMA table_info(sessions)').all().map((c) => c.name);
    if (columns.includes('expired')) return; // already the new shape
    db.exec('DROP TABLE sessions');
    console.log('Dropped the old sessions table (better-sqlite3-session-store shape) — connect-session-knex will recreate it; every existing session is now signed out.');
  }

  migrateSessionsTableForKnexStore();
}

module.exports = runLegacySqliteSchema;
