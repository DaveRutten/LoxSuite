...
async function loadMqttToLoxoneView() {
  return db.prepare(
...
async function loadTargetSuggestions() {
  return (await db.prepare('SELECT DISTINCT target FROM mappings_mqtt_to_loxone ORDER BY target').all()).map((r) => r.target);
}
...
async function loadLoxoneToMqttView(baseUrl) {
  ...
  return Promise.all(rows.map(async (row) => ({
    ...
    translationValues: row.value_transform === 'translation_table'
      ? (await loxoneTranslationValuesStmt.all(row.id)).map((t) => t.match_value)
      : null,
  })));
}

router.get('/mqtt-to-loxone', asyncHandler(async (req, res) => {
  const mappings = await loadMqttToLoxoneView();
  const miniservers = await db.prepare('SELECT * FROM miniservers ORDER BY sort_order, id').all();
  const targetSuggestions = await loadTargetSuggestions();
  res.render('mappings-mqtt-to-loxone', { mappings, miniservers, targetSuggestions, error: null, prefillTopic: req.query.topic || '' });
}));

router.get('/mqtt-to-loxone/data.json', asyncHandler(async (req, res) => {
  const canEdit = res.locals.canEdit('mqtt_to_loxone');
  const rows = (await loadMqttToLoxoneView()).map((m) => ({
    id: m.id,
    enabled: !!m.enabled,
...
router.post('/mqtt-to-loxone', requirePermission('mqtt_to_loxone', 'edit'), asyncHandler(async (req, res) => {
...
router.get('/mqtt-to-loxone/:id/edit', asyncHandler(async (req, res) => {
  const mapping = await db.prepare('SELECT * FROM mappings_mqtt_to_loxone WHERE id = ?').get(req.params.id);
...
router.post('/mqtt-to-loxone/:id/update', requirePermission('mqtt_to_loxone', 'edit'), asyncHandler(async (req, res) => {
  const { miniserver_id, mqtt_topic, transport, target, value_transform, transform_arg, min_interval_ms } = req.body;
  await db.prepare(
...
router.get('/mqtt-to-loxone/:id/translations', asyncHandler(async (req, res) => {
  ...
  const translations = await db.prepare('SELECT * FROM mapping_translations WHERE mapping_id = ? ORDER BY match_value').all(mapping.id);
  res.render('mapping-translations', { mapping, translations, error: null });
}));

router.post('/mqtt-to-loxone/:id/translations', requirePermission('mqtt_to_loxone', 'edit'), asyncHandler(async (req, res) => {
  const { match_value, output_value } = req.body;
  try {
    await db.prepare('INSERT INTO mapping_translations (mapping_id, match_value, output_value) VALUES (?, ?, ?)')
...
router.post('/mqtt-to-loxone/:id/translations/:translationId/delete', requirePermission('mqtt_to_loxone', 'edit'), asyncHandler(async (req, res) => {
...
router.post('/commands/catalog', requirePermission('commands', 'edit'), asyncHandler(async (req, res) => {
...
router.post('/commands/create-rgbw', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  ...
  const insertSql = `INSERT INTO mappings_loxone_to_mqtt (miniserver_id, token, mqtt_topic, qos, retain, transport, value_transform, transform_arg)
     VALUES (NULL, ?, ?, 0, 0, 'udp', 'shelly_rgbw', ?)`;
  await db.prepare(insertSql).run(nanoid(16), mqttTopic, rgbMode);
...
router.get('/loxone-to-mqtt', asyncHandler(async (req, res) => {
  ...
  const mappings = await loadLoxoneToMqttView(baseUrl);
  const miniservers = await db.prepare('SELECT * FROM miniservers ORDER BY sort_order, id').all();
...
router.get('/loxone-to-mqtt/data.json', asyncHandler(async (req, res) => {
  const baseUrl = `${req.protocol}://${req.get('host')}`;
  const canEdit = res.locals.canEdit('loxone_to_mqtt');
  const rows = (await loadLoxoneToMqttView(baseUrl)).map((m) => ({
    id: m.id,
    enabled: !!m.enabled,
    mqttTopic: m.mqtt_topic,
    miniserverName: m.miniserver_name,
...
router.post('/loxone-to-mqtt', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
...
router.get('/loxone-to-mqtt/:id/edit', asyncHandler(async (req, res) => {
  const mapping = await db.prepare('SELECT * FROM mappings_loxone_to_mqtt WHERE id = ?').get(req.params.id);
  if (!mapping) return res.status(404).send('Mapping not found');
  const miniservers = await db.prepare('SELECT * FROM miniservers ORDER BY sort_order, id').all();
  res.render('mapping-loxone-to-mqtt-edit', { mapping, miniservers, error: null });
}));

router.post('/loxone-to-mqtt/:id/update', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  const { miniserver_id, mqtt_topic, qos, retain, transport } = req.body;
  const { transform, transformArg } = normalizeLoxoneToMqttTransform(req.body);
...
router.post('/loxone-to-mqtt/:id/test', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  const mapping = await db.prepare('SELECT * FROM mappings_loxone_to_mqtt WHERE id = ?').get(req.params.id);
  ...
  if (mapping.transport === 'udp') {
    ...
    socket.send(Buffer.from(message), port, '127.0.0.1', async (err) => {
      socket.close();
      ...
      const expectedValue = await applyLoxoneToMqttTransform(mapping, rawValue);
      res.json({
        ok: true,
...
router.post('/loxone-to-mqtt/:id/toggle', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  await db.prepare('UPDATE mappings_loxone_to_mqtt SET enabled = 1 - enabled WHERE id = ?').run(req.params.id);
  res.redirect('/mappings/loxone-to-mqtt');
}));
...
router.post('/loxone-to-mqtt/disable-by-topic', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  const topic = (req.body.mqtt_topic || '').trim();
  if (topic) await db.prepare('UPDATE mappings_loxone_to_mqtt SET enabled = 0 WHERE mqtt_topic = ?').run(topic);
  res.redirect(req.get('referer') || '/logs/loxone-commands');
}));

router.post('/loxone-to-mqtt/enable-all', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  await db.prepare('UPDATE mappings_loxone_to_mqtt SET enabled = 1').run();
  res.redirect('/mappings/loxone-to-mqtt');
}));

router.post('/loxone-to-mqtt/disable-all', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  await db.prepare('UPDATE mappings_loxone_to_mqtt SET enabled = 0').run();
  res.redirect('/mappings/loxone-to-mqtt');
}));

router.post('/loxone-to-mqtt/:id/delete', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  await db.prepare('DELETE FROM mappings_loxone_to_mqtt WHERE id = ?').run(req.params.id);
  res.redirect('/mappings/loxone-to-mqtt');
}));

router.get('/loxone-to-mqtt/:id/translations', asyncHandler(async (req, res) => {
  const mapping = await db.prepare(
  ...
  const translations = await db.prepare('SELECT * FROM loxone_mapping_translations WHERE mapping_id = ? ORDER BY match_value').all(mapping.id);
  res.render('loxone-mapping-translations', { mapping, translations, error: null });
}));

router.post('/loxone-to-mqtt/:id/translations', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  const { match_value, output_value } = req.body;
  try {
    await db.prepare('INSERT INTO loxone_mapping_translations (mapping_id, match_value, output_value) VALUES (?, ?, ?)')
      .run(req.params.id, match_value, output_value);
  } catch (err) {
    // likely a duplicate match_value for this mapping — ignore, the UNIQUE constraint protects data integrity
  }
...
router.post('/loxone-to-mqtt/:id/translations/:translationId/delete', requirePermission('loxone_to_mqtt', 'edit'), asyncHandler(async (req, res) => {
  await db.prepare('DELETE FROM loxone_mapping_translations WHERE id = ? AND mapping_id = ?').run(req.params.translationId, req.params.id);
  res.redirect(`/mappings/loxone-to-mqtt/${req.params.id}/translations`);
}));
