// Fresh in-memory database per test run — exercises every CREATE TABLE / migration in
// legacy-sqlite-schema.js from scratch each time, which is as close as this suite gets to "does a
// brand new install boot cleanly" without actually starting the whole server.
...
test('a fresh database seeds an Administrator and a Viewer role', async () => {
  const roles = await db.prepare('SELECT name, is_admin FROM access_roles ORDER BY name').all();
...
test('the Administrator role has a permission row for every known area', async () => {
  const adminRole = await db.prepare('SELECT id FROM access_roles WHERE name = ?').get('Administrator');
  const rows = await db.prepare('SELECT area FROM access_role_permissions WHERE role_id = ?').all(adminRole.id);
...
test('singleton settings tables each have exactly one row', async () => {
  for (const table of ['gateway_settings', 'mqtt_settings', 'backup_settings', 'sso_settings']) {
    const count = (await db.prepare(`SELECT COUNT(*) AS c FROM ${table}`).get()).c;
...
test('the notification tables exist with the expected join table', async () => {
  assert.ok(await tableExists('notification_channels'));
  assert.ok(await tableExists('notification_rules'));
  assert.ok(await tableExists('notification_rule_channels'));
});

test('backup_settings carries the rclone columns', async () => {
  const columns = (await db.prepare('PRAGMA table_info(backup_settings)').all()).map((c) => c.name);
